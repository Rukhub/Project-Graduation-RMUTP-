...
class _EquipmentDetailScreenState extends State<EquipmentDetailScreen> {
  ...
  void initState() {
    ...
    permanentId =
        widget.equipment['permanent_id']?.toString() ??
        widget.equipment['permanentId']?.toString() ??
        widget.equipment['permanentID']?.toString();
    ...
    assetName =
        widget.equipment['asset_name']?.toString() ??
        widget.equipment['name_asset']?.toString();
      ...
      if (mounted) {
        setState(() {
          ...
          if (logs.isNotEmpty) {
            ...
            lastAuditNote = (latestLog['note'] ?? latestLog['remark'])
                ?.toString();
            ...
            if (latestFailedLog != null) {
              final nextFailedReason =
                  (latestFailedLog['note'] ?? latestFailedLog['remark'])
                      ?.toString();

              final nextFailedImage =
                  latestFailedLog['evidence_image']?.toString() ??
                  latestFailedLog['asset_image_url']?.toString();

              final nextFailedByName = latestFailedLog['auditor_name']
                  ?.toString();
            ...
            if (latestNormalLog != null) {
              lastNormalInspectorName = latestNormalLog['auditor_name']
                  ?.toString();

              lastNormalAuditNote =
                  (latestNormalLog['note'] ?? latestNormalLog['remark'])
                      ?.toString();
            ...
            if (latestDamagedLog != null) {
              lastDamagedInspectorName = latestDamagedLog['auditor_name']
                  ?.toString();

              lastDamagedAuditNote =
                  (latestDamagedLog['note'] ?? latestDamagedLog['remark'])
                      ?.toString();

              lastDamagedEvidenceImage =
                  latestDamagedLog['evidence_image']?.toString() ??
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final currentUserUid =
                    ApiService().currentUser?['uid'] ?? 'unknown_uid';
                ...
                if (prevId == null || prevId.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('ไม่พบรายการแจ้งซ่อมเดิมสำหรับอ้างอิง'),
                    ),
                  );
                ...
                if (pickedImage != null) {
                  uploadedReportImageUrl = await FirebaseService()
                      .uploadReportImage(pickedImage!, assetIdStr);
                }
  ...
  void _showReportProblemDialog() {
    ...
    final String currentUserName =
        ApiService().currentUser?['fullname'] ?? 'ไม่ระบุ';
    ...
    _latestReportStreamSub = FirebaseFirestore.instance
        ...
        .listen(
          (snapshot) {
            ...
            closed.sort((a, b) {
              final da =
                  _toDateTime(a['finished_at']) ??
                  _toDateTime(a['reported_at']) ??
                  DateTime.fromMillisecondsSinceEpoch(0);

              final db =
                  _toDateTime(b['finished_at']) ??
                  _toDateTime(b['reported_at']) ??
                  DateTime.fromMillisecondsSinceEpoch(0);

              return db.compareTo(da);
            });

            final Map<String, dynamic>? latestClosed = closed.isNotEmpty
                ? closed.first
                : null;
            ...
            if (active.isNotEmpty) {
              final pending = active
                  .where(
                    (r) =>
                        FirebaseService.reportStatusToCode(
                          r['report_status'],
                        ) ==
                        1,
                  )
              ...
              final bool isAssetRepairingNow =
                  equipmentStatus == 'อยู่ระหว่างซ่อม';
              ...
              for (final r in candidates) {
                final rr =
                    (r['report_remark'] ?? r['remark_report'])?.toString() ??
                    '';
            ...
            final String? fetchedReporterName = latest['reporter_name']
                ?.toString();
            ...
            if (isRepairAgain) {
              fetchedReportReason =
                  (latest['report_remark'] ?? latest['remark_report'])
                      ?.toString();
            } else {
              fetchedReportReason =
            ...
            final int reportStatusCode = FirebaseService.reportStatusToCode(
            ...
            final String? closedFinishedRemark = latestClosed == null
                ? null
                : (latestClosed['finished_remark'] ??
                          latestClosed['remark_finished'] ??
                          latestClosed['remark_completed'])
                      ?.toString();

            final String? closedFinishedImage = latestClosed == null
                ? null
                : (latestClosed['finished_image_url'] ??
                          latestClosed['finished_image'] ??
                          latestClosed['finished_imageUrl'])
                      ?.toString();

            final String? closedFinishedByName = latestClosed == null
                ? null
                : (latestClosed['worker_name'] ?? latestClosed['workerName'])
                      ?.toString();

            final DateTime? closedFinishedAt = latestClosed == null
                ? null
                : (_toDateTime(latestClosed['finished_at']) ??
                      _toDateTime(latestClosed['reported_at']));
            ...
            final String? fetchedFinishedRemark =
                (latest['finished_remark'] ??
                        latest['remark_finished'] ??
                        latest['remark_completed'])
                    ?.toString();

            final String? fetchedFinishedImage =
                (latest['finished_image_url'] ??
                        latest['finished_image'] ??
                        latest['finished_imageUrl'])
                    ?.toString();
            ...
            final String? fetchedFailedReason =
                (latest['finished_remark'] ??
                        latest['remark_finished'] ??
                        latest['remark_broken'] ??
                        latest['failed_reason'])
                    ?.toString();

            final String? fetchedFailedImage =
                (latest['finished_image_url'] ??
                        latest['broken_image_url'] ??
                        latest['failed_image_url'])
                    ?.toString();
            ...
            setState(() {
              ...
              if (reportStatusCode == 2) {
                if (fetchedWorkerName != null &&
                    fetchedWorkerName.trim().isNotEmpty) {
              ...
              if (reportStatusCode == 3 || reportStatusCode == 4) {
                if (fetchedFinishedRemark != null &&
                    fetchedFinishedRemark.trim().isNotEmpty) {
                  latestFinishedRemark = fetchedFinishedRemark;
                }

                if (fetchedFinishedImage != null &&
                    fetchedFinishedImage.trim().isNotEmpty) {
              ...
              if (closedFinishedRemark != null &&
                  closedFinishedRemark.trim().isNotEmpty) {
                latestFinishedRemark = closedFinishedRemark;
              }

              if (closedFinishedImage != null &&
                  closedFinishedImage.trim().isNotEmpty) {
                latestFinishedImageUrl = closedFinishedImage;
              }

              if (closedFinishedByName != null &&
                  closedFinishedByName.trim().isNotEmpty) {
            ...
            if (snapshot.exists && mounted) {
              ...
              if (rawStatus == 1 || rawStatus == '1' || rawStatus == 'ปกติ') {
                ...
                newStatus = 'ชำรุด';
                  ...
                  rawStatus == '3' ||
                  rawStatus == 'รอดำเนินการ') {
                if (rawRepairerId != null &&
                    rawRepairerId.toString().isNotEmpty) {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (myReports.isNotEmpty) {
          ...
          if (active.isNotEmpty) {
            final pending = active
                .where((r) => r['report_status']?.toString() == 'pending')
                .toList();
          ...
          final int latestReportStatusCode = FirebaseService.reportStatusToCode(
            latest['report_status'],
          );
          ...
          if (fetchedReporterName == null || fetchedReporterName == 'ไม่ระบุ') {
            ...
            if (rId != null) {
              final reporterUser = await FirebaseService().getUserProfileByUid(
                rId,
              );
        ...
        setState(() {
          ...
          brandModel =
              updatedAsset['brand_model'] ?? updatedAsset['brandModel'];

          assetName = updatedAsset['asset_name'] ?? updatedAsset['assetName'];

          permanentId =
              updatedAsset['permanent_id']?.toString() ??
              updatedAsset['permanentId']?.toString() ??
              updatedAsset['permanentID']?.toString();
          ...
          if (finalReporter == null || finalReporter == 'ไม่ระบุ') {
            finalReporter =
                (updatedAsset['reporter_name'] ?? updatedAsset['reporterName'])
                    ?.toString();
          }
          ...
          if (!hasReportDoc) {
            if (finalReason == null || finalReason.isEmpty) {
              finalReason =
                  (updatedAsset['issue_detail'] ?? updatedAsset['issueDetail'])
                      ?.toString();
            }
          ...
          if (hasReportDoc &&
              FirebaseService.reportStatusToCode(fetchedLatestReportStatus) ==
                  4) {
            if (fetchedFailedReason != null &&
                fetchedFailedReason!.trim().isNotEmpty) {
              failedReason = fetchedFailedReason;
    ...
    try {
      ...
      for (final path in imagePaths) {
        ...
        if (path.startsWith('http') || path.startsWith('https')) {
          ...
          final uploadedUrl = await FirebaseService().uploadAssetImage(
            file,
            assetId,
          );
  ...
  void _showStatusDialog() {
    ...
    debugPrint(
      "DEBUG: StatusDialog - Status: $equipmentStatus, Int: $currentStatusInt",
    );
  ...
  void _showAuditDialog() {
    ...
    String tempStatusText = equipmentStatus == 'ปกติ'
        ? 'ชำรุด'
        : equipmentStatus;
    ...
    final String currentUserName =
        ApiService().currentUser?['fullname'] ?? 'Admin';
    ...
    showDialog(
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (ctx, setDialogState) {
            final Color primaryColor = isRepairVerification
                ? const Color(0xFF5593E4)
                : const Color(0xFF3D7BC4);

            final Color gradientStartColor = isRepairVerification
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!isRepairVerification) ...[
                        ...
                        GestureDetector(
                          onTap: () =>
                              setDialogState(() => tempStatusText = 'ชำรุด'),
            ...
            return AlertDialog(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () =>
                            setDialogState(() => actionParams = 'success'),
                      ...
                      GestureDetector(
                        onTap: () =>
                            setDialogState(() => actionParams = 'fail'),
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: actionParams == 'success'
                        ? Colors.green
                        : Colors.red,
                  ),
  ...
  Future<void> _handleStatusChange(
    ...
    try {
      ...
      if (evidenceImage != null) {
        if (newStatus == 2) {
          evidenceUrl = await FirebaseService().uploadReportImage(
            evidenceImage,
            assetIdStr,
          );
        } else {
          evidenceUrl = await FirebaseService().uploadRepairImage(
            evidenceImage,
            assetIdStr,
          );
        }
        ...
        if (latestReportDocId != null && latestReportDocId!.trim().isNotEmpty) {
          try {
            ...
            if (existingName == null || existingName.trim().isEmpty) {
              final fallbackName =
                  (reporterName != null && reporterName!.trim().isNotEmpty)
                  ? reporterName!.trim()
                  : currentUserName;
              ...
              updateData['reporter_id'] =
                  (existingId != null && existingId.trim().isNotEmpty)
                  ? existingId.trim()
                  : currentUserUid;
            }
      ...
      if (isFinishRepair && newStatus == 1) {
        ...
        if (latestReportDocId != null && latestReportDocId!.trim().isNotEmpty) {
          ...
          try {
            ...
            final repairingDocs = snapshot.docs.where((d) {
              final data = d.data();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('บันทึกสถานะเรียบร้อย'),
          backgroundColor: Colors.green,
        ),
      );
  ...
  Future<void> _handleFailedRepair(
    String note,
    File image,
    BuildContext dialogContext,
  ) async {
    try {
      ...
      String? imgUrl = await FirebaseService().uploadRepairImage(
        image,
        assetIdStr,
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('บันทึกผลซ่อมไม่ได้เรียบร้อย'),
      ...
      child: Scaffold(
          ...
          leading: IconButton(
            ...
            onPressed: () {
              Navigator.pop(context, {
                ...
                'asset_image_url': imagePaths.isNotEmpty
                    ? imagePaths.first
                    : null,
        ...
        body: ListView(
          ...
          children: [
            ...
            if (equipmentStatus == 'ชำรุด' &&
                shouldPreferDamagedAuditSection) ...[
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (latestFinishedImageUrl != null &&
                      latestFinishedImageUrl!.trim().isNotEmpty) ...[
                    const SizedBox(height: 12),
      ...
      child: Column(
        ...
        children: [
          ...
          if (lastDamagedEvidenceImage != null &&
            ...
            GestureDetector(
              onTap: () =>
                  _showFullScreenImage(context, lastDamagedEvidenceImage!),
                ...
                child: ClipRRect(
                  ...
                  child: Image.network(
                    ...
                    errorBuilder: (ctx, err, stack) =>
                        const Center(child: Text('โหลดรูปภาพไม่สำเร็จ')),
                  ),
                ),
              ),
            ),
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow(
            ...
            (permanentId != null && permanentId!.trim().isNotEmpty)
                ? permanentId!
                : '-',
            ...
            children: [
              Icon(
                originalStatus == 'ปกติ'
                    ...
                    : originalStatus == 'ชำรุด'
                    ? Icons.error
                    : originalStatus == 'อยู่ระหว่างซ่อม'
                    ? Icons.handyman
                    : Icons.cancel,
                ...
                // 2. Check by Name (Fallback for legacy data)
                  ...
                  final byName =
                      (latestWorkerName != null &&
                          latestWorkerName!.trim().isNotEmpty)
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(headerIcon, color: themeColor, size: 24),
              ),
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (latestFinishedByName != null &&
                      latestFinishedByName!.trim().isNotEmpty) ...[
                    const SizedBox(height: 4),
