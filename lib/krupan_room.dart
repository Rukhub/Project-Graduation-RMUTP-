...
class _KrupanRoomScreenState extends State<KrupanRoomScreen> {
  ...
  Future<void> _navigateToInspect(
    ...
    final inspectorName =
        equipment['inspectorName'] ?? equipment['auditor_name'] ?? '';
            ...
            return AlertDialog(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    StreamBuilder<List<Map<String, dynamic>>>(
                      stream: FirebaseService().getPermanentAssetsStream(),
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (selectedPermanentId == null && groups.isNotEmpty) {
                          WidgetsBinding.instance.addPostFrameCallback((_) {
                            final preferred = groups.firstWhere(
                              (g) =>
                                  (g['permanent_id']?.toString() ??
                                      g['id']?.toString() ??
                                      '') ==
                                  '1206010101',
                            ...
                            setDialogState(() {
                              selectedPermanentId =
                                  preferred['permanent_id']?.toString() ??
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: DropdownButton<String>(
                                value: selectedPermanentId,

                                isExpanded: true,

                                underline: const SizedBox(),

                                hint: const Text('เลือกกลุ่มสินทรัพย์ถาวร'),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        helperText: '⚠️ ใช้ _ แทน / (ระบบจะแสดงเป็น / ให้)',
                ...
                ElevatedButton(
                  onPressed: (isSaving || selectedCategory == null)
                      ...
                      : () async {
                          ...
                          if (selectedPermanentId == null ||
                              selectedPermanentId!.trim().isEmpty) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('กรุณาเลือกกลุ่มสินทรัพย์ถาวร'),
                          ...
                          final permanentError = await FirebaseService()
                              .validatePermanentIdForNewAsset(
                                selectedPermanentId!,
                              );
                          ...
                          final available = await FirebaseService()
                              .isAssetIdAvailable(assetId);

                          String finalAssetId = assetId;

                          if (!available) {
                            final status = await FirebaseService()
                                .getPermanentStatus(selectedPermanentId!);

                            if (status == 2) {
                              final generated = await FirebaseService()
                                  .generateNextAvailableAssetId(assetId);
                              ...
                              if (generated == null) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                      'ไม่สามารถสร้างรหัสครุภัณฑ์อัตโนมัติได้ กรุณาลองรหัสอื่น',
                                    ),
                              ...
                              finalAssetId = generated;

                              idController.text = generated;
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    'รหัสซ้ำ ระบบตั้งรหัสเป็น "$generated" อัตโนมัติ',
                                  ),
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text(
                                    'รหัสครุภัณฑ์นี้มีอยู่แล้ว กรุณาตรวจสอบและแก้ไขให้ไม่ซ้ำ',
                                  ),
                  ...
                  ElevatedButton(
                    onPressed: isSaving
                        ...
                        : () async {
                            ...
                            if (selectedImage != null) {
                              final uploadedUrl = await FirebaseService()
                                  .uploadAssetImage(
                                    selectedImage!,
  ...
  Future<void> _showMoveDialog() async {
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        ...
        Widget _floorChip(StateSetter setSheetState, String floor) {
  ...
  Future<void> _executeMoveAssets(
    ...
    try {
      ...
      await FirebaseService().moveAssetsToLocation(
        ...
        targetLocationName: targetRoomName,
      );
